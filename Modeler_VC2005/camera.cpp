...
void Camera::releaseMouse( int x, int y )
{
	...
	//初始默认相机位置变量
	Vec3f temp_mPosition;	
	Vec3f temp_mLookAt;
	Vec3f temp_mUpVector;
...
void Camera::applyViewingTransform() {
	...
	temp_mPosition[0]=mPosition[0];
	temp_mPosition[1]=mPosition[1];
	temp_mPosition[2]=mPosition[2];
	temp_mLookAt[0]=mLookAt[0];
	temp_mLookAt[1]=mLookAt[1];
	temp_mLookAt[2]=mLookAt[2];
	temp_mUpVector[0]=mUpVector[0];
	temp_mUpVector[1]=mUpVector[1];
	temp_mUpVector[2]=mUpVector[2];

	if(VAL(DEFAULT_CAMERA_POINT)==1)	//恢复相机默认位置
	{
		gluLookAt(9,9,23,0,0,0,0, 1,0);
		
		//cout<<int(temp_mPosition[0])<<endl;
		//cout<<int(temp_mPosition[1])<<endl;
		//cout<<int(temp_mPosition[2])<<endl;
		//cout<<int(temp_mLookAt[0])<<endl;
		//cout<<int(temp_mLookAt[1])<<endl;
		//cout<<int(temp_mLookAt[2])<<endl;
		//cout<<int(temp_mUpVector[0])<<endl;
		//cout<<int(temp_mUpVector[1])<<endl;
		//cout<<int(temp_mUpVector[2])<<endl;
	}
	else	//相机被移动后的位置
